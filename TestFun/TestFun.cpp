...
//std::map<char, double> FreqFirstLetter = {
...
int HashMap(std::string word, int num_reducer) {
	...
	try {
		AssignedReducer = (int)FreqFirstLetter.at(std::tolower(*(word.begin()), loc)) * num_reducer;
	}
...
int main()
{
	...
	while (file >> word)
	{
		...
		std::cout << word << ', index of its reducer is ' << HashMap(word, 8) << std::endl;
	}
